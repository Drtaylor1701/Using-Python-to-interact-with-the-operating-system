#!/usr/bin/env python3
'''notes on reading files video'''
#to open a program called spider.txt
file = open("spider.txt")
'''
creates a new file object and assigns it a variable called "file"
the parameter passed to open() is the name of the file we want to open
we are assuming it's in the same directory we're working in
when we open the file:
1. the OS checks that we have permission to open the file and then 
2. gives the code a file descriptor

file descriptor
-token
-generated by the OS
-allows programs to do more operations with the file
